...
const AddSupplier = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
       await axios.post(`/FOURNISSEUR/AJOUTER`, {
        forminput
      }).then( async (response) => {
        const identifiant = response.data;
          await axios.post(`/UPLOAD-IMAGE/UPLOADIMGSUPPLIER/${identifiant}`, formData)
          Succes("Supplier add with Success !","success");
      })
    } catch (error) {
      console.log(error);
      return Error("This Supplier is Exist", "error");
